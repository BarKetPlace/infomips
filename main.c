...
int execute_cmd(interpreteur inter, virtualmem vm, Liste_int* pbreaklist) {
	...
	if(strcmp(token, "run") == 0){ fct_exec(0x0, vm);
...
int main ( int argc, char *argv[] ) {
   ...
   // DEBUG_MSG("Un message DEBUG_MSG !"); /* macro DEBUG_MSG : uniquement si compil en mode DEBUG_MSG */
	...
	//printf("%d\n", known_cmd(inter->input));
            ...
            int res = execute_cmd(inter, vm, pbreaklist); /* execution de la commande */
