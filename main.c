...
int execute_cmd(interpreteur inter, mem memory, registre* reg, Liste dico, Liste_int breaklist) {
	...
	if(!strcmp(token, "break")){
		return breakcmd(inter,memory, breaklist);
	}
...
int main ( int argc, char *argv[] ) {
   ...
   // DEBUG_MSG("Un message DEBUG_MSG !"); /* macro DEBUG_MSG : uniquement si compil en mode DEBUG_MSG */
	...
	//printf("%d\n", known_cmd(inter->input));
            ...
            int res = execute_cmd(inter, memory, reg, dico, breaklist); /* execution de la commande */
